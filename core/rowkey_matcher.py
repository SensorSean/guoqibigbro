...
# 通用前后缀（V1.2.8）：这些词几乎出现在所有本地项目中，不能作为项目身份。
# 旧策略4 直接用 _FLEX_TOKENS（含 本地/项目/建设…）做集合重合，导致
...
def _norm_core(s: str) -> str:
    ...
    s = re.sub(r'[（(][^）)]*[）)]', '', s)
    # 范围连接词「至/到」：真正的区间端点连接词（「A至B」「A到B」），
    # 尾部端点无条件截断，使同一区间的不同写法收敛到同一个键。
    s = re.sub(r'[至到][^之及至到]*$', '', s)
    # 范围连接词「之/及」：*仅当* 尾部是一个「区段/端点」标记（段 / 标段 / 区间 / 幅）
    # 时才截断，例如「A路之B段」→「A路」、「示范路及示范北路段」→「示范路」。
    # 之/及 在中文里同时是高频并列连词，不可无条件截断——详见 normalize() 文档串。
    s = re.sub(r'[之及][^之及至到]*?(?:段|区间|幅)$', '', s)
    s = re.sub(r'[\s_\-]', '', s)
    return s.lower()
...
class RowKeyMatcher:
    ...
    def normalize(self, s: str) -> str:
        ...
        范围连接词截断分两类处理（V1.3.3 收窄，修复「归一化正则灾难性误删」）：

        1. 至 / 到 —— 真正的区间端点连接词（「A至B」「A到B」），尾部端点**无条件**截断，
           使同一区间的不同写法收敛到同一个键。
        2. 之 / 及 —— **仅当**尾部是一个「区段/端点」标记（段 / 标段 / 区间 / 幅）时才截断，
           例如「A路之B段」→「A路」、「示范路及示范北路段」→「示范路」。

        为什么要拆开：之/及 在中文里同时是高频**并列连词**，不是范围连接词。旧规则
        ``r'[之及至到][^之及至到]*$'`` 把连接词之后到串尾的整段无条件删掉，会摧毁并列
        结构的后半段，把不同的项目塌陷成同一个键，进而在 score() 里被判为归一化后精确
        相等（1.0），使 align() 的高分优先贪心 1:1 无法区分它们而循环互换（静默写错值）。

        已实证的塌陷案例：
          - 「示范城市之光医院门诊综合楼二期项目」「示范城市之光商业综合体项目」
            「示范城市之光写字楼三期项目」→ 旧规则三者全部变成「示范城市」。
          - 「示例学校」与「示例学校及周边道路填土夯实工程」→ 旧规则都变成「示例学校」。
          - 「示范新城片保障性住房(一期)」与「示范新城片保障性住房(二期)及配套道路」
            → 旧规则都变成「示范新城片保障性住房」。

        收窄后上述误删全部消失，而 V1.2.5 引入的「A路之B段 → A路」能力
        （避免「B段」反向误匹配主项目）完整保留——该能力无法由 _road_endpoints 接管，
        因为 _ROAD_RE 只解析「某路（A-B）」这类**括号形态**，不匹配无括号的「A路之B段」。
    ...
    def _strip_generics(s: str) -> str:
        ...
        示范南片三期）。通用词（本地/项目/建设/开发…）几乎出现在所有
        本地项目中，不能作为项目身份。
        """
        core = s
        changed = True
    ...
    def score(self, a: str, b: str) -> float:
        ...
        # 旧逻辑直接用 _FLEX_TOKENS（本地/项目/建设…）做集合重合，导致
        # 「某市旅游基础设施」与「某市省级森林公园保护性开发」
        # 因共享通用词「本地/项目」被误判为同一项目（0.9 分）。
    ...
    def align(self, tgt_keys: List[str], src_keys: List[str]) -> Dict[str, str]:
        ...
        # 源键（如 "示范南路" 与 "示范南路（...）"）会被合并，破坏 1:1 贪心分配。
